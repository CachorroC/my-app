...
export function Actuaciones (
  { actuaciones }: { actuaciones: Actuacion[] }
) {
  return (
    <div className={ box.grid }>{
      actuaciones.map(
        ( actuacion: Actuacion ) => (
          <ActuacionCard
            key={ actuacion.consActuacion } actuacion={ actuacion }
          />
        )
      )
    }</div>
...
export function ActuacionCard ( {
  ...
  return (
    <div className={ layout.card } key={ actuacion.consActuacion }>
      <h1 className={ poiret.className }>{ actuacion.actuacion }</h1>
      <p className={ typeface.block }>{ actuacion.anotacion }</p>
